function verifyPositions(a, b, c, d) {
  1 == b &&
    $("#checkPopBox .checkbox input").attr("checked") &&
    $.cookie("verifyPos", 1, { expired: 30 }),
    $.ajax({
      type: "POST",
      url: ctx + "/corpPosition/verifyPositions.json",
      data: { ids: a, type: b, reason: c, resubmitToken: d },
      dataType: "json",
    }).done(function (a) {
...
$(function () {
  $(".job_del").bind("click", function () {
    ...
    return confirm("")
      ? ($.ajax({
          url: ctx + "/corpPosition/delete.json",
          data: { positionId: b, resubmitToken: c },
          dataType: "json",
        }).done(function (b) {
    ...
    $(".job_offline").click(function (a) {
      var b, c, d;
      return (
        a.preventDefault(),
        (b = $(this)),
        (c = b.parents("li").attr("data-id")),
        (d = $("#resubmitToken").val()),
        confirm("")
          ? ($.ajax({
              url: ctx + "/corpPosition/offline.json",
              data: { id: c, resubmitToken: d },
              dataType: "json",
            }).done(function (a) {
              $("#resubmitToken").val(a.resubmitToken),
                a.success
    ...
    $(".links .job_refresh").bind("click", function (a) {
      var b, c;
      a.preventDefault(),
        (b = $(this)),
        (c = b.parents("li").attr("data-id")),
        $.ajax({
          url: ctx + "/corpPosition/refresh.json",
          data: { id: c },
          dataType: "json",
        }).done(function (a) {
          a.success
            ? (b.parent(".links").siblings("div.c9").html(""),
              b
                .html(
                  "<span></span><i></i>"
                )
                .attr("class", "job_refreshed"),
              b.children("i").fadeIn(200).delay(1500).fadeOut(200),
              b.unbind("click"))
            : alert(a.msg);
        });
    }),
    ...
    $("#filterForm").on("mouseenter", ".checkbox", function () {
      $(this).children("input").attr("checked") ||
        $("i", this).stop(!0, !0).fadeIn(200);
    }),
    $("#filterForm").on("mouseleave", ".checkbox", function () {
      $(this).children("input").attr("checked") ||
        $("i", this).stop(!0, !0).fadeOut(200);
    }),
    $(".resumeLists").on("click", ".job_verify", function () {
      ...
      1 == $.cookie("verifyPos")
        ? ((a = $(this).parents("li").attr("data-id")),
          (b = $("#resubmitToken").val()),
          verifyPositions(a, 1, "", b))
        : ((c = $(this).parent()),
          $(".company_center_content").find(".check_popbox").remove(),
          $("#checkPopBox").clone().appendTo(c),
          c
            .children("#checkPopBox")
            .addClass("check_popbox_verify")
            .removeClass("dn"));
    }),
    $(".resumeLists").on("click", ".job_verify_fail", function () {
      var a = $(this).parent();
      $(".company_center_content").find(".check_popbox").remove(),
        $("#uncheckPopBox").clone().appendTo(a),
        a
          .children("#uncheckPopBox")
          .addClass("check_popbox_unverified")
          .removeClass("dn");
    }),
        ...
        "" == a
          ...
          : (1 == $.cookie("verifyPos")
              ...
              : ((c = $(this).parent()),
                ...
                c
                  .children("#checkPopBox")
                  .addClass("check_popbox_verifyAll")
                  .removeClass("dn")),
            void 0)
      );
    }),
    $("#failAll").bind("click", function () {
      ...
      return (
        ...
        "" == a
          ...
          : ((b = $(this).parent()),
            $(".company_center_content").find(".check_popbox").remove(),
            $("#uncheckPopBox").clone().appendTo(b),
            b
              .children("#uncheckPopBox")
              .addClass("check_popbox_unverifiedAll")
              .removeClass("dn"),
            void 0)
      );
    }),
    $("#filterForm").on(
      "click",
      ".check_popbox .close,.check_popbox .cancel",
      function () {
    ...
    $(".resumeLists").on("click", "#checkPopBox .btn_confirm", function () {
      var a = $("#resubmitToken").val(),
        b = $(this).parents("li").attr("data-id");
      verifyPositions(b, 1, "", a);
    }),
    $(".filter_actions").on(
      "click",
      "#uncheckPopBox .btn_confirm",
      function () {
        var a = new Array(),
          b = $("#resubmitToken").val(),
          c = $(this).parent().find('input[type="radio"]:checked').val();
        $(".resumeLists li").each(function () {
          $(this).find('input[type="checkbox"]').attr("checked") &&
            a.push($(this).attr("data-id"));
        }),
          (a = a.join(",")),
          c ? verifyPositions(a, 3, c, b) : alert("");
      }
    ),
    $(".resumeLists").on("click", "#uncheckPopBox .btn_confirm", function () {
      var a = $("#resubmitToken").val(),
        b = $(this).parents("li").attr("data-id"),
        c = $(this).parent().find('input[type="radio"]:checked').val();
      c ? verifyPositions(b, 3, c, a) : alert("");
    }),
    $(".filter_actions").on(
      "click",
      ".check_popbox .checkbox input",
      function () {
        return (
          $(this).attr("checked")
            ? ($(this).removeAttr("checked"),
    ...
    $("#job_preview_btmbar").on("click", ".btn_verify", function () {
      var a, b;
      $.cookie("verifyPos") && 1 == $.cookie("verifyPos")
        ? ((a = $("#positionId").val()),
          (b = $("#resubmitToken").val()),
          verifyPositions(a, 1, "", b))
        : ($(this)
            .siblings("#uncheckPopBox")
            .removeClass("check_popbox_unverified")
            .addClass("dn"),
          $(this)
            .siblings("#checkPopBox")
            .addClass("check_popbox_verify")
            .removeClass("dn"));
    }),
    $("#job_preview_btmbar").on(
      "click",
      "#checkPopBox .btn_confirm",
      function () {
        var a = $("#resubmitToken").val(),
          b = $("#positionId").val();
        verifyPositions(b, 1, "", a);
    ...
    $("#job_preview_btmbar").on("click", ".btn_unverify", function () {
      $(this)
        .siblings("#checkPopBox")
        .removeClass("check_popbox_verify")
        .addClass("dn"),
        $(this)
          .siblings("#uncheckPopBox")
          .addClass("check_popbox_unverified")
    ...
    $(
      "#job_preview_btmbar .check_popbox .close,#job_preview_btmbar .check_popbox .cancel"
    ).click(function () {
    ...
    $("#job_preview_btmbar .checkbox").hover(
      function () {
        $(this).children("input").attr("checked") ||
          $("i", this).stop(!0, !0).fadeIn(200);
      },
